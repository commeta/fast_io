...
0. **31:** Номер блокировки. Это уникальный идентификатор для каждой блокировки.
1. **FLOCK:** Тип блокировки. Возможные значения:
...
2. **ADVISORY:** Режим блокировки. Возможные значения:
...
3. **WRITE:** Тип доступа. Возможные значения:
   - READ: Блокировка на чтение.
   - WRITE: Блокировка на запись.
4. **1542032:** PID процесса, который установил блокировку.
5. **fc:01:4760679:** Идентификатор файла, который заблокирован. Этот идентификатор состоит из трех частей:
   - fc: Мажорный номер устройства.
   - 01: Минорный номер устройства.
   - 4760679: Номер inode файла.
6. **0:** Начало диапазона блокировки (смещение в байтах от начала файла).
7. **EOF:** Конец диапазона блокировки (EOF указывает на конец файла).
