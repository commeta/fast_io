...
1. file_replace_line() — sizeof(pointer) вместо размера буфера
```c
// БЫЛО: fread(dynamic_buffer, 1, sizeof(dynamic_buffer), temp_fp)
// СТАЛО: fread(dynamic_buffer, 1, dynamic_buffer_size, temp_fp)
```

2. replicate_file() — та же проблема sizeof(pointer) в двух циклах fread для source_fp и index_source_fp.

3. replicate_file() — неинициализированные переменные index_source_fp/index_destination_fp объявлены с NULL за пределами блока if(mode == 1), чтобы код обработки ошибок emalloc не обращался к неинициализированным указателям.

4. file_defrag_lines() — пропущен unlink(temp_filename) в ветке mode=1 при некоторых путях ошибок.

5. file_search_array() — непоследовательная логика found_count в режимах >19: found_count++ перемещён внутрь блока is_matched, как и в остальных режимах.

6. file_callback_line() — буфер не сбрасывается при jump При переходе на новую позицию через jump=1: добавлен сброс current_size = 0 и dynamic_buffer[0] = '\0', иначе уже прочитанные строки из буфера обрабатывались повторно.

7. file_analize() — mode=1 не возвращает min_length_offset и max_length_offset — поля добавлены в массив результата согласно документации.

8. file_select_array() — mode=6 не фильтровал по pattern (аналог mode=5 но для "line"). Добавлена проверка strstr(buffer, pattern).

9. file_get_keys() — утечка памяти при mode=5: line_arr создавался, но не использовался и не освобождался. При mode=5 создание line_arr пропускается.

10. file_analize() — min_length инициализируется SIZE_MAX, но правильно обнуляется при отсутствии строк — это поведение корректное, исправление касается mode=1 (пункт 7).
