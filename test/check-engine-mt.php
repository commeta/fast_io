 ...
 *
 * check-engine-mt.php — МНОГОПРОЦЕССНЫЙ СТРЕСС-ТЕСТ fast_io v3.0
 *
 * Покрытие:
 ...
 *   - Консистентность данных между параллельными процессами
 *   - Статистика ядра Linux (/proc/PID/io) по каждому тесту
 *
 * Запуск: php check-engine-mt.php [workers=12] [iterations=120]
 *
...
define('DEBUG', false);   // подробный вывод ошибок
define('SHOW_IO_PER_PHASE', true);  // вывод статистики ядра после каждой фазы

$workers = isset($argv[1]) ? max(2, (int) $argv[1]) : 12;
$iterations = isset($argv[2]) ? max(10, (int) $argv[2]) : 120;

$base_dir = sys_get_temp_dir() . '/fast_io_mt_' . getmypid();
@mkdir($base_dir, 0755, true);

// Файлы теста
$DB_MAIN = $base_dir . '/main.dat';           // insert/select/update/search
$DB_STACK = $base_dir . '/stack.dat';          // pop_line (LIFO)
$DB_DATA = $base_dir . '/data.dat';           // push_data / search_data
$DB_REPLICA = $base_dir . '/replica.dat';        // replicate_file
$DB_CALLBACK = $base_dir . '/callback.dat';       // callback_line
$CONSISTENCY_MAP = $base_dir . '/consistency.map';    // консистентность: offset→expected
$RESULT_DIR = $base_dir . '/results';            // результаты дочерних процессов
...
// Запись результата дочернего процесса
function child_result(string $result_dir, int $worker_id, array $data): void
{
    $path = $result_dir . '/w' . $worker_id . '.json';
    file_put_contents($path, json_encode($data, JSON_PRETTY_PRINT));
}

// Чтение всех результатов
function read_results(string $result_dir, int $workers): array
{
    $all = [];
    for ($i = 0; $i < $workers; $i++) {
        $path = $result_dir . '/w' . $i . '.json';
        if (file_exists($path)) {
            $all[$i] = json_decode(file_get_contents($path), true) ?? [];
        }
    }
    return $all;
}

// Атомарная запись в map-файл консистентности (offset => payload)
function consistency_register(string $map_file, int $offset, int $align, string $payload): void
{
    // Запись: "offset:align:payload_hex\n"
    $line = $offset . ':' . $align . ':' . bin2hex($payload) . "\n";
    $fp = fopen($map_file, 'a+');
    if ($fp) {
        flock($fp, LOCK_EX);
        fwrite($fp, $line);
...
function consistency_read_all(string $map_file): array
{
...
echo "╔══════════════════════════════════════════════════════════════════╗\n";
echo "║      fast_io МНОГОПРОЦЕССНЫЙ СТРЕСС-ТЕСТ  v3.0                  ║\n";
echo "╚══════════════════════════════════════════════════════════════════╝\n\n";
...
//  Родитель вставляет $workers * $iterations строк с фиксированным
//  выравниванием. Каждый дочерний процесс получает свой диапазон
//  смещений и знает точное ожидаемое содержимое.
// ═══════════════════════════════════════════════════════════════════
...
$offsets_map = [];   // offset => trim_payload (для проверки консистентности)
...
for ($i = 0; $i < $TOTAL_LINES; $i++) {
    ...
    if ($off % $ALIGN !== 0) {
        echo "⚠️  Некратный offset=$off (не multiple of $ALIGN)\n";
    }
    $offsets_map[$off] = $trim_pl;
    consistency_register($CONSISTENCY_MAP, $off, $ALIGN, $trim_pl);
}

// Подготовка файла для LIFO-теста
for ($i = 0; $i < $workers * 4; $i++) {
    $s = sprintf('stack_item_%05d_%s', $i, str_pad('', 40, 'S'));
    file_insert_line($DB_STACK, $s, 2, $ALIGN);
}
...
// ═══════════════════════════════════════════════════════════════════
//  РАСПРЕДЕЛЕНИЕ ДИАПАЗОНОВ ПО РАБОЧИМ ПРОЦЕССАМ
//  Каждый воркер получает свой набор смещений (без пересечений),
//  чтобы обновления не конкурировали за одни и те же строки.
//  Дополнительно: все воркеры параллельно читают весь файл —
//  это проверяет конкурентные чтения.
// ═══════════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════════
//  ФАЗЫ 1–N — ДОЧЕРНИЕ ПРОЦЕССЫ
// ═══════════════════════════════════════════════════════════════════

phase_header('PHASE 1 — Параллельное выполнение (все воркеры)');
...
for ($w = 0; $w < $workers; $w++) {
    ...
    if ($pid === 0) {
        // ─── ДОЧЕРНИЙ ПРОЦЕСС ───────────────────────────────────────
        mt_srand(getmypid() ^ ($w * 1000031) ^ (int) (microtime(true) * 1e6));
        $wid = $w;
        $my_offs = $worker_ranges[$w];  // мои строки без пересечений
        $result = [
            'wid' => $wid,
            'pid' => getmypid(),
            'errors' => [],
        ...
        $erased_offsets = []; // если захочешь потом отслеживать

        // Локальная функция ошибки
        $err = function (string $phase, string $msg) use (&$result): void {
            $result['errors'][] = "[$phase] $msg";
            if (DEBUG)
                echo "[W{$result['wid']} ERR] [$phase] $msg\n";
        };

        ini_set('fast_io.buffer_size', mt_rand(4096, 131072));

        // ── SUB-TEST A: SELECT (чтение своих строк) ──────────────────
        $io_a = get_io_stats();
        $ta = microtime(true);
        $ok_a = true;
        ...
        // ── SUB-TEST B: SELECT ARRAY (массовое чтение) ───────────────
        $io_b = get_io_stats();
        $tb = microtime(true);
        $ok_b = true;
        ...
        // ── SUB-TEST C: UPDATE_LINE (обновление своих строк) ─────────
        $io_c = get_io_stats();
        $tc = microtime(true);
        $ok_c = true;
        $updated_c = [];  // off => new_expected
        ...
        // ── SUB-TEST E: SEARCH_LINE (полнотекстовый поиск) ───────────
        $io_e = get_io_stats();
        $te = microtime(true);
        $ok_e = true;
        // Ищем строку по уникальному ключу воркера
        ...
        // Тест на отдельном файле — не трогаем $DB_MAIN
        $io_h = get_io_stats();
        $th = microtime(true);
        $ok_h = true;
        ...
        // Стираем несколько строк по ключу
        for ($ei = 5; $ei < 15; $ei++) {
        ...
        // Убеждаемся что flow_interruption == 0 (нет обрванных записей)
        ...
        // Убеждаемся match_offset корректен
        ...
        // У каждого воркера свой отдельный мини-файл для replace
        $io_m = get_io_stats();
        $tm = microtime(true);
        $ok_m = true;
        ...
        // Каждый воркер делает push→pop в собственный временный стек
        $io_n = get_io_stats();
        $tn = microtime(true);
        $ok_n = true;
        ...
        child_result($RESULT_DIR, $wid, $result);
...
// ═══════════════════════════════════════════════════════════════════
//  ФАЗА 2 — КОНСИСТЕНТНОСТЬ ДАННЫХ МЕЖДУ ПРОЦЕССАМИ
//  Читаем CONSISTENCY_MAP и проверяем, что текущие данные в файле
//  корректны: каждая ячейка либо содержит оригинальный контент,
//  либо обновлённый (но не мусор и не нулевые байты).
// ═══════════════════════════════════════════════════════════════════

phase_header('PHASE 2 — Консистентность данных (родитель читает после всех воркеров)');

$t2 = microtime(true);
$io2 = get_io_stats();
$cons = consistency_read_all($CONSISTENCY_MAP);
$ok_cons = true;
$checked = 0;
$corrupted = [];
...
foreach ($cons as $off => $meta) {
    $checked++;

    // mode 1 = байтовый offset + trimmed (без trailing \n/spaces)
    $trimmed = file_select_line($DB_MAIN, $off, $meta['align'], 1);

    // mode 3 = байтовый offset + RAW (ровно align байт)
    $raw = file_select_line($DB_MAIN, $off, $meta['align'], 3);
    ...
    if ($raw === false || strlen($raw) !== $meta['align']) {
        $corrupted[] = "offset=$off: raw_len=" . strlen($raw ?? '') . " expected={$meta['align']}";
        $ok_cons = false;
        continue;
    }
    // Первый байт не должен быть нулевым
...
if (!empty($corrupted)) {
    foreach (array_slice($corrupted, 0, 10) as $c)
        echo "  ⚠️  $c\n";
}
...
phase_header('PHASE 4 — Сводная статистика по воркерам');

$all_results = read_results($RESULT_DIR, $workers);
...
// Заголовок таблицы
printf("\n  %-22s", 'SUB-TEST');
for ($w = 0; $w < min($workers, 12); $w++)
    printf("  W%-2d", $w);
echo "  PASS/TOTAL\n";
echo '  ' . str_repeat('─', 22 + min($workers, 12) * 5 + 12) . "\n";
...
// ═══════════════════════════════════════════════════════════════════
//  ФАЗА 5 — СУММАРНАЯ СТАТИСТИКА ВВ/ВВ ЯДРА LINUX
// ═══════════════════════════════════════════════════════════════════

phase_header('PHASE 5 — Суммарная статистика ввода/вывода (родительский процесс)');
...
foreach ($sub_tests as $st) {
    ...
    for ($w = 0; $w < $workers; $w++) {
        ...
        if (isset($r['io_stats'][$st])) {
            foreach ($io_keys as $ik) {
                $agg_io[$st][$ik] += $r['io_stats'][$st][$ik] ?? 0;
            }
        }
        if (isset($r['ops'][$st]['t'])) {
            $agg_t += $r['ops'][$st]['t'];
    ...
    $rchar_m = round($agg_io[$st]['rchar'] / 1e6, 2);
    $wchar_m = round($agg_io[$st]['wchar'] / 1e6, 2);
    $syscr = $agg_io[$st]['syscr'];
    $syscw = $agg_io[$st]['syscw'];
    printf(
...
echo "  Воркеры (всего/упало)     : {$workers} / {$failed_workers}\n";
...
if ($overall_ok) {
    echo "✅  check-engine-mt.php — УСПЕШНО ПРОЙДЕН\n";
    echo "    Библиотека fast_io корректно работает в многопроцессной среде.\n";
    echo "    Данные консистентны. Блокировки работают верно.\n";
} else {
    echo "❌  ОБНАРУЖЕНЫ ПРОБЛЕМЫ:\n";
    if ($failed_workers > 0)
        echo "    - Упало воркеров: {$failed_workers}\n";
    if (count($corrupted) > 0)
        echo "    - Повреждённых ячеек: " . count($corrupted) . "\n";
    if ($total_errors > 0)
        echo "    - Ошибок операций: {$total_errors}\n";
    if (!$ok_f3)
        echo "    - file_analize финального файла провалена\n";
    if (!$ok_cons)
        echo "    - Консистентность данных нарушена\n";
}
