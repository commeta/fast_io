...
Из результатов анализа Valgrind видно, что не было обнаружено явных утечек памяти (definitely lost), косвенных утечек (indirectly lost) или возможных утечек (possibly lost) во время выполнения скрипта fast_io.php, который использовал расширение fast_io.so.
