...
- если $end отрицательное число, тогда $end будет количеством байт от конца файла.


Когда параметр $end < 0 меньше нуля, file_pop_line меняет знак числа $end на противоположный и отнимает $end от размера файла.
```
