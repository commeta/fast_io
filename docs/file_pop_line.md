...
- возвращает $offset строк от нового конца файла, строка после $new_file_size отбрасывается
- в режиме усечения файла строка после $new_file_size тоже будет удалена из файла
