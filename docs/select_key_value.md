...
В режиме mode=1 можно указать любой размер строки index_align, строка будет обрезана по символу перевода строки. Если символ перевода строки не будет наеден, функция вернет подстроку.
