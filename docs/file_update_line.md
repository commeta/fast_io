...
int file_update_line(string $filename, string $line, int $line_row, int $line_align [, int mode = 0])
...
- line_row - целое число, номер строки (записи), которую необходимо обновить. Нумерация начинается с 0.
- line_align - целое число, размер одной записи в файле (включая перенос строки).
- mode (int, optional) - Режим адресации: 0 по номеру строки, 1 по смещению строки.
