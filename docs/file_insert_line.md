...
В этом примере мы пытаемся добавить строку "file_insert_line_ [авто заполнитель]" в файл fast_io1.dat, при этом каждая запись должна занимать ровно 8192 символов. Если операция проходит успешно, функция вернёт номер строки, в которую была добавлена запись. В случае ошибки будет возвращён соответствующий код.
